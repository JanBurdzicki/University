...
;; detranslation -> TODO: do dodalismy

#;(define (n-th-variable n)
  (if (< n 1)
      "x"
      (string-append (n-th-variable (- n 1)) "1")))

(define (variable n)
  (string->symbol (string-append "x" (to-string n))))

(define (detranslate [e : ExpA] [env-n : Number]) : Exp
  (type-case ExpA e
    [(numA n)
     (numE n)]
    [(opA o l r)
     (opE o (detranslate l env-n) (detranslate r env-n))]
    [(ifA b l r)
     (ifE (detranslate b env-n)
          (detranslate l env-n)
          (detranslate r env-n))]
    [(varA n)
     (varE (variable (- (- env-n n) 1)))]
    [(letA e1 e2)
     (letE (variable env-n)
           (detranslate e1 env-n)
           (detranslate e2 (+ env-n 1)))]))

; (variable 2137)
; (variable 42)


(define (run-det [s : S-Exp]) : Exp
  (detranslate (translate (parse s) mt-envA) 0))
...
(module+ test
  ...
  (test (run `{let x 1
        ...
        (runA `{let x 1
                 {+ x {let x {+ x 1}
                        {* x 3}}}}))
  (test (run-det `{let x 1 {+ x {let y 2 {* x y}}}})
        (letE 'x0 (numE 1) (opE (add) (varE 'x0) (letE 'x1 (numE 2) (opE (mul) (varE 'x0) (varE 'x1)))))))