...
(define (apply-all [exp : Exp] [xs : (Listof Exp)]) : Exp
  (if (empty? xs)
      exp
      (appE (apply-all exp (rest xs)) (first xs))))

(define (lambda-all [xs : (Listof Exp)] [exp : Exp]) : Exp
  (if (empty? xs)
      exp
      (lamE (varE-x (first xs)) (lambda-all (rest xs) exp))))
...
(define (parse [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{lambda {ANY ANY ...} ANY} s) ; lambda ma przyjmowac minimum 1 rzecz
     (lambda-all (parse-all (s-exp->list
                             (second (s-exp->list s))))
    ...
    [(s-exp-match? `{ANY ...} s) ; co najmniej jedna rzecz aplikuje
     (apply-all (parse (first (s-exp->list s)))
                (reverse (parse-all (rest (s-exp->list s)))))]
