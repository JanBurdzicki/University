...
(define (returnM [v : 'a]) : (M 'a)
  (λ ([s : State]) (pair (valE v) s)))

(define (bindM [c : (M 'a)] [f : ('a -> (M 'b))]) : (M 'b)
  (λ (s)
    (let ([c*s (c s)])
      (type-case (E 'a) (fst c*s)
        [(valE val)
         ((f val) (snd c*s))]
        [(errE l m)
         (pair (errE l m) (snd c*s))]))))
...
(define (showM [c : (M Value)]) : String
  (let* ([c*s (c 0)]
    ...
    (type-case (E 'a) new-c
      [(valE val)
       (string-append "value: "
       (string-append (value->string val)
       (string-append ", state: "
                      (to-string new-s))))]
      [(errE l m)
       (string-append "error in "
    ...
    [(appE e0 e1)
     (bindM (eval e0 env)
            (λ (v0) (bindM (eval e1 env)
                           (λ (v1) (bindM (apply v0 v1)
                                          (λ (v2) (bindM tickM
                                                         (λ (v3) (returnM v2)))))))))]
    [(countE)
     (λ (s) (pair (valE (numV s)) s))]))
...
(module+ test
   ...
   (test (run `{+ 0 {lambda {x} x}})
          "error in prim-op: not a number, state: 0")
