...
1#
Niech a=(list v) oraz niech (f (list v)) oznacza wykonanie funkcji na liście (skrót myślowy, bo rekurencją ogonową przechodzi po liście)
...
3#
(bindM (bindM c f) g) ≡ (bindM c (λ (a) (bindM (f a) g)))
(bindM (f (list v)) g) ≡ (bindM (list v) (λ (a) (bindM (f a) g))) => ((λ (a) (bindM (f a) g)) (list v)) => (bindM (f (list v)) g)
(g (f (list v))) ≡ (g (f (list v)))
-> skrót myślowy, bo dostajemy listę xs = (f (list v)), wykonujemy (bind xs g), dostajemy ys = (g xs), co jest równe (g (f (list v)))
|#
